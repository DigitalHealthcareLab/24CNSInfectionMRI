...
class ApproxExplainerW():
    ...
    def __init__(self, torchmodel, tensordataset, featurevector_sizes, unitsize = 1, method='regression', num_to_sample=None, sample_batch_size=None, nan=0, tolerance = 0.05, device='cuda'):
        ...
        if not isinstance(unitsize, int):
            try:
                unitsize.to(self._device)
            except:
                raise ValueError('unitsize must be an integer or a tensor')
        ...
        self._shap_weighted = self._shap_values / (self._featurevector_sizes.to(self._device) / unitsize)
