...
def calculate_binary_accuracy(outputs, targets):
    with torch.no_grad():
        batch_size = targets.size(0)

        # Assuming outputs are logits (before applying sigmoid)
...
def add_and_shuffle2(X1, X2, y1, y2, random_state = 42):
    torch.manual_seed(random_state)
    X_temp = torch.cat([X1, X2], dim = 0)
    y_temp = torch.cat([y1, y2], dim=0)
